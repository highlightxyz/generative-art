...
The main goal of **Highlight Studio** is to let the artist quickly try out different parameters that affect their artworks, see multiple previews and to get a sense of how their traits distribution (chance of occurring) looks like.

The app can be used completely standalone, although with limited functionality, but that might be enough for your use case. In the following section we will describe what core functionality the app provides and how to make the most of it.

For more advanced usage and insight it is highly recommended to download the [hl-studio.js](./hl-studio.js) script, and import it right after `hl-gen.js` in your html file. This script takes over the globally exposed `hl object` that hl-gen provides, proxies it, and sends information to Highlight Studio for processing.

Things like **“trait harvester”** which looks at your artwork and runs it until it gets the trait information, need a bit of modification to your code if you would like to get a large sample size (10-100k). But don’t worry, the modification is a best practice anyway, so doing it will make your code easier to reason about.
...
While Highlight Studio can be used with your artwork just as-is, and it tries to do some clever tricks to make sure your experience is smooth and performant, it is ultimately up to you, the artist, to help Highlight Studio be the best it can be.

This means adding short circuits to your code, which allows for faster executions, omitting code that’s not relevant (e.g. applying an expensive filter over your image is redundant in the case where you are just testing which image gets selected).

To do this Highlight Studio adds another flag on the `hl.context` object called `studioMode`. This flag is set **ONLY** when loading your artwork from the Highlight Studio and when the Studio Mode flag in the form is set. This means you can do things like this:
...
This will just draw a background color and skip any other elements. If you can, always do this. Surgically remove parts of the code you don’t need for testing, and in turn all the operations (specifically Multiple artwork preview and Trait harvester) will work much faster.
...
Now let's get into it! The features of Highlight Studio, how to use them, what you need to use them and some tips and tricks.
...
HL Studio works by embedding your project in an iframe (just like [Highlight.xyz](https://highlight.xyz/) does!) and giving you tools to play around with the artwork. To get started, you need to set the URL that points to the location of your project. Artists usually have a build process in place, where they can quickly iterate on their artwork, by hosting it locally and using features like hot-reload.

If you are one of them... great! Just point to your local server and you're good to go!

If not, don't worry. It's super simple. Just navigate to the root folder of your project, open the command line, and run (this assumes you have node installed, if not check here):
...
There is also a new flag you can hook into called `“Studio mode”`, which you can use to make sure studio specific code is executed only when using Highlight Studio. In the future custom events and triggers that you define will be captured by Highlight Studio allowing you to do more in-depth and custom analysis of your code, but I digress.
...
This tool can be used without `hl-studio.js` script, but the performance will be worse and you will miss out on token details, but if all you want is to see how your artworks look, it might do the job just fine!
...
With these options you can now program how certain values change over time. In the future there will be more customization options (like randomly/sequentially picking from a list of provided values)
Underneath the form there are generation options. Let's go through them:

### Number of previews

How many generations to do? Be careful with large numbers, especially if you’re not using `hl-studio.js` script to generate screenshots of your live artworks.
...
This function is used by Highlight studio to quickly harvest traits from your artwork. It expects a `hl` object which you should use to set all the variables your artwork needs. Then return an object containing the traits like so:
...
Technically you can also emit your own events and Highlight studio will pick them up (but do nothing with them), but this also means you can play around with how events Highlight studio does listen to, get emitted. For example you can short circuit the Multiple artwork preview `replace-with-screenshot` generation by just emitting the event yourself in some if block, checking if studio mode flag is set, allowing you to leave your actual artwork code intact and just adding a block specific to when Highlight studio is running.
